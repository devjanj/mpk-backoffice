...
async function inspect() {
    try {
        ...
        const validRows = rows.filter(r => r && r[0] && String(r[0]).trim() !== '');
        
        console.log("Last 2 valid rows:");
        for(let i=Math.max(0, validRows.length - 2); i<validRows.length; i++) {
           console.log(JSON.stringify(validRows[i]));
        }
    } catch (e) {
